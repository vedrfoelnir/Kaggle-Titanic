...
object NaiveBayes {
  ...
  def countAttributeValues(data:List[Map[String, Any]], attribList:String): Map[Any, Int]=
    data
      .flatMap(X => X.filter(_._1 == attribList).values) // get clean list of attribute values
      .groupBy(identity).mapValues(_.size) // count values
  ...
  def getAttributes(data:List[Map[String, Any]]):Set[String] =
    data.flatMap(X => X.keys).groupBy(identity).keys.toSet
  ...
  def calcAttribValuesForEachClass(data:List[Map[String, Any]], classAttrib:String): Map[Any, Set[(String, Map[Any, Int])]] =
    countAttributeValues(data, classAttrib)
      .map(X => X._1 -> data.filter(_(classAttrib)==X._1)
        .flatMap(_.filterNot(_._1==classAttrib))
        .groupBy(_._1)
        .map(X => (X._1, X._2.map(V => V._2)))
        .map(X => (X._1, X._2.groupBy(identity).mapValues(X => X.size))).toSet)
  ...
  def calcConditionalPropabilitiesForEachClass(data: Map[Any, Set[(String, Map[Any, Int])]],classCounts:Map[Any,Int]): Map[Any,Set[(String, Map[Any, Double])]] =
    data.map(X => (X._1, X._2.map(V => V._1 -> V._2
        .mapValues(_.asInstanceOf[Double]/classCounts.filter(_._1==X._1).values.sum))))
  ...
  def calcClassValuesForPrediction(record:Map[String,Any], conditionalProps: Map[Any,Set[(String, Map[Any, Double])]], priorProps:Map[Any,Double]):Map[Any,Double]= {
    val getProps = priorProps
      .flatMap(X => conditionalProps
        .filter(_._1==X._1)
        .mapValues(X => {
            for (el <- record) yield (X.map(V => if(el._1==V._1) V._2.getOrElse(el._2, 0.0))).toList}.flatten))
      .mapValues(_.filter(_.isInstanceOf[Double]).toSeq.toList.fold(1.0)((k,v) => k.asInstanceOf[Double]*v.asInstanceOf[Double]).asInstanceOf[Double])
    priorProps.map(X => X._1 -> X._2*getProps(X._1))

    // The story of stubbornly trying to use a bad algorithm that spits out AnyVals...
  }
   ...
   *         conditional propability stored in a map(second element).
   */
  def calcConditionalPropabilitiesForEachClassWithSmoothing(data: Map[Any, Set[(String, Map[Any, Int])]],
                                                            attValues:Map[String,Set[Any]],
                                                            classCounts:Map[Any,Int]): Map[Any,Set[(String, Map[Any, Double])]] = {
    val aVals = attValues.mapValues(_.size)
    val imputeMissing = data.map(X => (X._1, X._2.map(V => V._1 -> attValues.filter(_._1==V._1).values.flatMap(_.flatMap(C => V._2.updated(C, V._2.getOrElse(C, 0)))).toMap)))
    imputeMissing.map(X => (X._1, X._2.map(V => V._1 -> V._2.mapValues(_+1))))
      .map(X => (X._1, X._2.map(V => V._1 -> V._2.mapValues(_.asInstanceOf[Double]/classCounts.filter(_._1==X._1).values.map(_+aVals(V._1)).sum))))
  }
