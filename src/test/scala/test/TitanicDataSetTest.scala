...
class TitanicDataSetTest extends AnyFunSuite {
  ...
  test("create DataSet for Training"){
    ...
    println(data(100))
    assert(s===891)
    assert(res===Map("age" -> 0, "sex" -> 0, "passengerID" -> 0, "pclass" -> 0, "survived" -> 0))
    //missing values derived from .size ... => 0 when no missing values
  }

  test("Create Model with Titanic Dataset"){

    val trainData = TitanicDataSet.createDataSetForTraining(train)
    val model= TitanicDataSet.createModelWithTitanicTrainingData(trainData, "survived")
    //age should be categorized
    val person1= Map[String,Any]("passengerID" -> 6, "sex" -> "male", "pclass" -> 3,"age"->"old")
